...
def short_url(longurl, attempt=0, alias=None):
    if not shorteners_list:
        return longurl
    if attempt >= 4:
        return longurl
    ...
    try:
        if "shorte.st" in _shortener:
            ...
            alias_param = f"&alias={alias}" if alias else ""
            return cget(
                'GET',
                f'https://modijiurl.com/api?api={_shortener_api}&url={quote(longurl)}{alias_param}'
            ).json().get('shortenedUrl', longurl)
        else:
            res = cget('GET', f'https://{_shortener}/api?api={_shortener_api}&url={quote(longurl)}').json()
            ...
            if not shorted:
                ...
                shrtco_link = shrtco_res['result']['full_short_link']
                res = cget('GET', f'https://{_shortener}/api?api={_shortener_api}&url={shrtco_link}').json()
                shorted = res.get('shortenedUrl', longurl)
            return shorted
    except Exception as e:
        LOGGER.error(e)
        sleep(1)
